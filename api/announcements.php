...
/* --- Όρια τηλεμετρίας. Ο πελάτης ΔΕΝ είναι αξιόπιστος. --------------- */
const ANN_TICK_MAX  = 30;    // δευτ. ανά κλήση· ο client στέλνει ανά 10-15
...
switch ($action) {
    ...
    case 'tick': {
        ...
        // Ο client μπορεί να πει ψέματα. Κόβουμε και ανά κλήση και συνολικά.
        $sec = max(0, min($sec, ANN_TICK_MAX));
        if ($sec === 0) respond(['ok' => true, 'tracked' => true]);

        $d->prepare("UPDATE `4a_announcement_reads`
                        SET `dwell_sec`   = LEAST(`dwell_sec` + ?, ?),
                            `last_seen_at`= NOW()
                      WHERE `announcement_id` = ? AND `user_id` = ?")
          ->execute([$sec, ANN_TOTAL_MAX, $aid, $uid]);

        respond(['ok' => true, 'tracked' => true]);
    }
