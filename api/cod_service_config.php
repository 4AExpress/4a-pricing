...
require_user();
$DEFAULTS = ['flat_limit' => 500.00, 'flat_fee' => 3.00, 'min_fee' => 1.30, 'percentage' => 1.00];

// Το enablement (επιτρέπει COD ή όχι) ΔΕΝ ζει εδώ — μοναδική πηγή είναι το
// `4a_services.has_cod`. Η στήλη cod_enabled παραμένει στον πίνακα αλλά ούτε
// διαβάζεται ούτε γράφεται πια: οι τιμές της είναι 1 παντού και δεν ισχύουν.
if ($method === 'GET') {
    $code = strtoupper(trim($_GET['service_code'] ?? ''));
    if (!$code) respond(['error' => 'service_code required'], 400);

    $stmt = db()->prepare("SELECT service_code, flat_limit, flat_fee, min_fee, percentage, updated_at
                           FROM cod_service_config WHERE service_code = ?");
    $stmt->execute([$code]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$row) {
        respond(array_merge($DEFAULTS, ['service_code' => $code, 'updated_at' => null]));
    }
    respond($row);
}
...
if ($method === 'POST') {
    ...
    db()->prepare("
        INSERT INTO cod_service_config (service_code, flat_limit, flat_fee, min_fee, percentage)
        VALUES (?, ?, ?, ?, ?)
    ...
    ")->execute([$code, $flat_limit, $flat_fee, $min_fee, $percentage]);
