#!/usr/bin/env python3
# generate_pdf.py | v1.1 | 18-05-2026
...
def _anchor_canvas_maker(registry):
    class _C(_rl_canvas.Canvas):
        def bookmarkPage(self, key, **kw):
            registry[key] = self._pageNumber
            super().bookmarkPage(key, **kw)
    return _C
...
def generate(offer_data, dhl_data, fuel_data):
    ...
    def build_story(toc_pages=None):
        story=[]
        ...
        # Pass 2: override estimated pages with actual anchor positions
        if toc_pages:
            toc_items = [(toc_pages.get(code, epg), code, desc, typ)
                         for epg, code, desc, typ in toc_items]
    ...
    doc_args = dict(pagesize=A4,
        rightMargin=MR, leftMargin=ML, topMargin=15*mm, bottomMargin=12*mm,
        title=f"Προσφορά {offer_num}", author='4A Express Worldwide')

    # Pass 1: dry-run to capture actual anchor → page mappings
    tmp1 = tempfile.NamedTemporaryFile(suffix='.pdf', delete=False)
    tmp1.close()
    anchor_pages = {}
    SimpleDocTemplate(tmp1.name, **doc_args).build(
        build_story(), canvasmaker=_anchor_canvas_maker(anchor_pages))
    os.unlink(tmp1.name)

    # Pass 2: rebuild TOC with real page numbers, produce final PDF
    tmpfile = tempfile.NamedTemporaryFile(suffix='.pdf', delete=False)
    tmpfile.close()
    SimpleDocTemplate(tmpfile.name, **doc_args).build(build_story(anchor_pages))
